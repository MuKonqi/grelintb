...
echo -e "GrelinTB and it's resetter licensed under GPLv3."
echo -e "\nCopyright (C) 2024 MuKonqi (Muhammed S.)"
echo -e "\nGrelinTB and it's resetter are free software: you can redistribute it and/or modify"
...
echo -e "\nGrelinTB and it's resetter are distributed in the hope that they will be useful,"
...
if  [ -f /etc/debian_version ]; then
    ...
    dnf reinstall --nogpgcheck grelintb.rpm -y
    rm grelintb.rpm
elif  [ -f /etc/solus-release ]; then
    reset
elif  [ -f /etc/arch-release ]; then
...
echo -e "GrelinTB has been reset. Exiting with status 0..."
exit 0